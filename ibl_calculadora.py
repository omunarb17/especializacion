...
from interpolaciones import calcular_ipc_inicial_interpolado, calcular_ipc_final_interpolado

def generar_tabla_periodos(fecha_inicio, fecha_fin, df_ipc, periodos_laborales):
    """Genera una tabla con los periodos desglosados por mes incluyendo datos de IPC y cotización con interpolación."""
    periodos = []
    fecha_actual = fecha_inicio

    # Calcular IPC Final una sola vez usando la fecha de fin seleccionada por el usuario
    ipc_final = calcular_ipc_final_interpolado(fecha_fin, df_ipc)
    ...
    while fecha_actual <= fecha_fin:
        ...
        for periodo in periodos_laborales:
            if periodo['fecha_inicio'] <= fecha_actual <= periodo['fecha_fin']:
                desde = periodo['fecha_inicio'] if fecha_actual < periodo['fecha_inicio'] else desde
        ...
        semanas_cotizadas = round(dias_cotizados / 7, 2)
        ...
        # Calcular IPC Inicial individualmente para cada mes
        ipc_inicial = calcular_ipc_inicial_interpolado(desde, df_ipc)
        ...
        periodos.append({
            ...
            "Salario": f"${salario_base:,.2f}", "Días Cotizados": dias_cotizados, "Semanas Cotizadas": semanas_cotizadas,
...
def calcular_resumen_ibl(df_periodos):
    ...
    total_semanas_cotizadas = df_periodos["Semanas Cotizadas"].sum()
    ...
    return pd.DataFrame({
        ...
        "📊 Semanas Cotizadas": [f"{total_semanas_cotizadas:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")],
...
num_periodos = st.number_input("Número de periodos laborales", min_value=1, max_value=10, value=1)
